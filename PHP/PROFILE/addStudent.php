    ...
    <main>
        <div class="addStudent-container">
            <?php
                ...
                function sendEmail($studentEmail,$studentName,$password) {
                    $mail = new PHPMailer();
                    $mail->isSMTP();
                    ...
                    if($mail->send()) {
                        echo '<div class="alert alert-success alert-dismissible fade in">
                                ...
                                <strong> Mail sent to ' . $studentEmail . ' </strong>
                                </div>';
                    }
                    else {
		    ...
		    if($_SESSION['designation'] == '4') {
                        ...
                        $staffDepartment = mysqli_fetch_array(mysqli_query($link,$sql));
			
                    }
                    if($_SESSION['designation'] == '1') {
                        $sql = "SELECT * FROM Staff WHERE staffEmail = '$staffEmail';";
                        $result = mysqli_fetch_array(mysqli_query($link,$sql));
                        $staffDepartment = $result['department'];
                        $staffSection = $result['section'];
                    }
                    
                    $json_data = file_get_contents('https://script.google.com/a/macros/kongu.edu/s/AKfycbwUUWGbIElhOhOSEwkHgJQQWLlzBo7-AQ_84OcqjQaVcyrRxvr1GQvwmaNXptCIHx-_/exec');
                
                    $response_data = json_decode($json_data);                    
                    foreach ($response_data as $user) {
                        $rollNumber = $user->rollno;
                        $studentName = strtolower($user->name);
                        ...
                        if($rollNumber != "" && $studentName != "" && $studentEmail != "" && $studentBatch != "" && $department != "" && $section != "" && $semester != "") {

                            if($_SESSION['designation'] == '4') {
                                if($staffDepartment[0] == $department) {
                                    ...
                                    if(mysqli_num_rows($result) > 0) {
                                        echo '<div class="alert alert-danger alert-dismissible fade in">
                                                ...
                                                <strong> ' . $studentEmail . ' already exists!</strong>
                                                </div>';
                                    }
                                    else {
                                        $sql = "INSERT INTO Student (rollNumber,studentName,studentEmail,studentBatch,department,section,currentSemester,passcode,verifyToken) VALUES ('$rollNumber','$studentName','$studentEmail','$studentBatch','$department','$section','$semester','$passcode','$verifyToken');";
                        	
                                	    if(mysqli_query($link,$sql)) {
                                	        sendEmail($studentEmail,$studentName,$password);
                                	    }
					    else {
                                    ...
                                    echo '<div class="alert alert-danger alert-dismissible fade in">
                                                <a href="#" class="close cancel" data-dismiss="alert" aria-label="close">&times;</a>
                                                <strong>Unable to add ' . $rollNumber . '! Please check the department</strong>
                                                </div>';
                                }
                            }
                            
                            if($_SESSION['designation'] == '1') {
                                if($staffDepartment == $department && $staffSection == $section) {
                                    ...
                                    echo '<div class="alert alert-danger alert-dismissible fade in">
                                                <a href="#" class="close cancel" data-dismiss="alert" aria-label="close">&times;</a>
                                                <strong>Unable to add ' . $rollNumber . '! Please check the department and section and batch.</strong>
                                                </div>';
                                }
                            }
                            if($_SESSION['designation'] == '5') {
                    	     ...
                    	     if($rollNumber == "" && $studentName == "" && $studentEmail == "" ) {
                    	         echo '<div class="alert alert-danger alert-dismissible fade in">
                                            <a href="#" class="close cancel" data-dismiss="alert" aria-label="close">&times;</a>
                                            <strong>Unable to add student ! Please check the credentials</strong>
                                            </div>';
                    	     }
                    	     else {
                    	         if($rollNumber != "") {
                    	            echo '<div class="alert alert-danger alert-dismissible fade in">
                                            <a href="#" class="close cancel" data-dismiss="alert" aria-label="close">&times;</a>
                                            <strong>Unable to add ' . $rollNumber . ' ! Please check the credentials</strong>
                                            </div>';
                    	         }
                    	         else if($studentName != "") {
                    	             echo '<div class="alert alert-danger alert-dismissible fade in">
                                            <a href="#" class="close cancel" data-dismiss="alert" aria-label="close">&times;</a>
                                            <strong>Unable to add ' . $studentName . ' ! Please check the credentials</strong>
                                            </div>';
                    	         }
                    	         else if($studentEmail != "") {
                    	             echo '<div class="alert alert-danger alert-dismissible fade in">
                                            <a href="#" class="close cancel" data-dismiss="alert" aria-label="close">&times;</a>
                                            <strong>Unable to add ' . $studentEmail . ' ! Please check the credentials</strong>
                                            </div>';
                    	         }
                    	     }
                    	}
